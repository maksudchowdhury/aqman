...
window.mathquill4quill = function(dependencies) {
  ...
  function enableMathQuillFormulaAuthoring(quill, options) {
    ...
    function areAllDependenciesMet() {
      ...
      // below section creates problem with the quill 2.0 editor hence rewritten in next if block
      ...
      if (!quill.options.modules.toolbar.container[0].includes('formula')) {
        console.log("Formula module not enabled"); // eslint-disable-line no-console
        return false;
      } 
    ...
    const historySize = options.historySize || 15;
